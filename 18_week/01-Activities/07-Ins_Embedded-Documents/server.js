...
app.get('/sale-over-30', (req, res) => {
  db.collection('groceryList')
    ...
    // .find({ "promotion.percentage_discount": 30 })
    ...
    //section not working. only works for objects not in array; keeps arrays if has a value that satisfy the condition but also keeps all other array conditions
    // $gte specifies we want percentage discounts greater than 30
    .find({ 'promotion.percentage_discount': { $gte: 30 } })

    //section this works for all items $gte: 30 or whatever condition
    ...
    .aggregate([
      { $project: { promotion: 1 } },
      { $unwind: "$promotion" },
      { $match: { "promotion.percentage_discount": { $gte: 30 } } },
    ])
